...
namespace AuthGG
{
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "binderror":
                            ...
                            return;
                        case "banned":
                            Console.WriteLine("This application has been banned for violating the TOS" + Environment.NewLine + "Contact us at support@auth.gg", Name);
                            Process.GetCurrentProcess().Kill();
                            return;
                    }
                    Security.End();
    ...
    internal class API
    {
        public static void Log(string username, string action)
        {
            ...
            {
                Console.WriteLine("Missing log information!", ApplicationSettings.Name);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
            {
                ...
                {
                    Console.WriteLine(ex.Message, OnProgramStart.Name);
                    Process.GetCurrentProcess().Kill();
                }
            }
        }
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "invalid_license":
                            Console.WriteLine("License does not exist!", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "email_used":
                            Console.WriteLine("Email has already been used!", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "invalid_username":
                            Console.WriteLine("You entered an invalid/used username!", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                    }
    ...
    internal class Security
    {
        ...
        public static void End()
        {
            if (!Constants.Started)
            {
                Console.WriteLine("No session has been started, closing for security reasons!", OnProgramStart.Name);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
    ...
    class InfoManager
    {
        ...
        private void OnCallBack()
        {
            ...
            {
                Constants.Breached = true;
                Console.WriteLine("ARP Cache poisoning has been detected!", OnProgramStart.Name);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
